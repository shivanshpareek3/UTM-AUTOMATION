...
def test_mapping_scenarios():
    ...
    meta_a = pd.DataFrame({'campaign': ['camp1'], 'spend': [100.0], 'Day': ['2026-08-15']})
    metrics_a, _, _ = run_pipeline(leads_df, sales_df, [meta_a], settings, 'output_a.xlsx')
    
    meta_b = pd.DataFrame({'campaign': ['camp1'], 'spend': [100.0], 'Day': ['2026-08-15']})
    metrics_b, _, _ = run_pipeline(leads_df, sales_df, [meta_b], settings, 'output_b.xlsx')
    
    meta_c = pd.DataFrame({'campaign': ['camp1'], 'spend': [100.0], 'Day': ['2026-08-15']})
    metrics_c, _, _ = run_pipeline(leads_df, sales_df, [meta_c], settings, 'output_c.xlsx')
    
    meta_d = pd.DataFrame({'campaign': ['camp1'], 'spend': [100.0], 'Amount Spent': ['Not a number'], 'Day': ['2026-08-15']})
